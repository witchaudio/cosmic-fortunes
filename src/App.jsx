...
function App() {
  ...
  return (
    <div className="app min-h-screen bg-gray-100">
      <p className="text-center text-base md:text-lg lg:text-xl">Select your astrological sign:</p>
      <div id="parent" className="max-w-lg mx-auto px-4">
        <div id="button-container" className="flex flex-wrap justify-center mt-4 md:mt-8">
          {astrologicalSigns.map((sign) => (
            <button
              key={sign}
              onClick={() => showHoroscope(sign)}
              className="text-sm md:text-base lg:text-lg xl:text-xl flex items-center justify-center"
            >
              <img src={iconLinks[sign]} alt={sign} className="w-4 h-4 mr-1" />
              <span>{sign}</span>
            </button>
          ))}
        </div>
        <div id="card-container" className="bg-white rounded-lg shadow-lg mt-8"></div>
        <div id="horoscope" className="text-center mt-8 text-base md:text-lg lg:text-xl xl:text-2xl"></div>
        <div id="compatibility" className="text-center text-base md:text-lg lg:text-xl xl:text-2xl"></div>
      </div>
    </div>
  );
}
